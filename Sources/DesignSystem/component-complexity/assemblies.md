...
```tsx
// Assembly - Product-specific flow using system components
...
## Pitfalls When Assemblies Creep Into Systems

1. **Assembly Drift**: one team's flow becomes a "component" in the system,
   and suddenly the system is maintaining product-specific features.

2. **Over-standardization**: forcing every product to use the same assembly
   pattern, even when needs diverge.

3. **System Bloat**: the system grows too heavy, making adoption harder
   rather than easier.
...
Assemblies are built using [primitives](/blueprints/component-standards/component-complexity/primitives),
